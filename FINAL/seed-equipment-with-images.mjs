...
const BACKUP_MEDIA_DIR = process.env.BACKUP_MEDIA_DIR || 
  (fs.existsSync('/app/backup/media') 
    ? '/app/backup/media' 
    : path.join(__dirname, 'av-rentals-backup-complete-20260109_152432', 'media'));
...
async function seedEquipmentWithImages() {
  ...
  try {
    ...
    if (!fs.existsSync(BACKUP_MEDIA_DIR)) {
      ...
      console.log('Looking in:', BACKUP_MEDIA_DIR);
      return;
    }
    ...
    for (let i = 0; i < imageFiles.length; i++) {
      ...
      try {
        ...
        failed++;
        console.error(`✗ ${fileName} - Error: ${error instanceof Error ? error.message : error}`);
      }
    }
